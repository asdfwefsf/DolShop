...
2. 결제 화면 만들고 UI 기깔나게 만들기
3. 마이페이지 -> 결제 정보 -> 주문내역(상품명 , 상품 갯수 , 가격 , 보내야 되는 계좌번호) 렌더링
4. 주문자의 입금관련 정보 (송신 은행명 , 송신 계좌번호 , 송신자 성함) -> Firebase 저장하기
5. 주문자의 입금관련 정보 (송신 은행명 , 송신 계좌번호 , 송신자 성함) -> Android System 가져오기
