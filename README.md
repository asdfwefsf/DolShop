...
2024-05-25
1. 상품 상세화면 UI 기깔나게 만들기
2. 결제 화면 만들기 (완료)
3. 결제 화면UI 기깔나게 만들기
4. 마이페이지 -> 결제 정보 -> 주문내역(상품명 , 상품 갯수 , 가격 , 보내야 되는 계좌번호) 렌더링
5. 주문자의 입금관련 정보 (송신 은행명 , 송신 계좌번호 , 송신자 성함) -> Firebase 저장하기
6. 주문자의 입금관련 정보 (송신 은행명 , 송신 계좌번호 , 송신자 성함) -> Android System 가져오기
7. 구매화면 -> 기존 주소 가져오기 + a : 애니메이션 (완료)
8. 구매화면 -> 새로운 주소 입려하기
9. 
