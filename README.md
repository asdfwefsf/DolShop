...
1. 유저정보가 기본값으로 파이어베이스 저장 -> 로그인 성공시 플로우 콜렉트 해서 비동기 처리에 조건문 설정하여 해결
2. 좋아요 클릭시 joayo 노드를 삭제하던 기존의 방식 -> 해당 유저 식별번호만 joayo 노드에서 삭제
3. 마이 다이어리 -> 퍼블릭 다이어리 이동 시 유저 네임 안 보여 -> PublicDiary 객체에 별도로 username 넣어주어 해결
4. 퍼블릭 다이어리에 있는 내 다이어리 삭제 기능 추가
