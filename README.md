...
2024-05-29
1. 로그인 화면 -> 회원가입 화면 -> 동의 할 때 개인정보 처리방침 , 서비스 이용약관 누르면 : 각각 웹뷰로 이동 (완료) : 노션X : 내 사이트에 링크
2. 회원가입1 : 개인정보 처리방침 , 서비스 이용약관 동의해야지 회원가입2 화면으로 넘어갈 수 있도록. (완료)

2024-05-30
1. 회원가입2 : 사용자 정보 입력 후 파이어베이스에서 회원가입 하는 로직
2. 회원가입3 : 회원가입 완료 시 보여주는 화면에서 로그인 관련 안내
3. 파이어베이스 회원가입 && 카카오 회원가입 로직 통합
